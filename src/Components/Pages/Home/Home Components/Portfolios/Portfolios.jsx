        ...
        <div className='py-28 md:py-32 bg-black'>
            <div className='w-11/12 m-auto text-center sm:text-left'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 gap-10 pt-10'>
                    {
                        portfolios.slice(0, 2).map(portfolio => <CardPF key={portfolio._id} portfolio={portfolio}/>)
                    }
                </div>
                <motion.div
                        initial={{ opacity: 0, y: 100 }}
            ...
            <div className="overflow-hidden border h-[32rem] border-white/30 text-center p-7 pb-0 rounded-3xl group/pfCard pt-16">
                ...
                <div className='translate-y-[0%] group-hover/pfCard:translate-y-[-20%] duration-500 ease-in'>
                    <div className='flex gap-2 items-center justify-center mb-5'>
                        ...
                        Case Study <span aria-hidden="true"><ArrowUpRightIcon className="h-4 w-4 2xl:h-5 2xl:w-5 relative text-white group-hover/pfCard:text-black duration-500 ease-in ml-1  "/></span>
                        </Link>
                        {/* Live Link Button */}
                        <Link to={portfolioLiveLink} target='_blank' className=" group mt-5 flex items-center text-sm 2xl:text-lg relative leading-6 text-white uppercase group-hover/pfCard:bg-white group-hover/pfCard:text-black group-hover/pfCard:px-6 group-hover/pfCard:py-2 rounded-full duration-500 ease-in">
                        Live <span aria-hidden="true"><ArrowUpRightIcon className="h-4 w-4 2xl:h-5 2xl:w-5 relative text-white group-hover/pfCard:text-black duration-500 ease-in ml-1 "/></span>
                        </Link>           
                    </div>
                    <img src={portfolioImgLink} alt="" srcSet="" className='rounded-t-xl '/>
                </div>
            </div>
        </>
    )
